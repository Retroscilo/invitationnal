    ...
    switch (id) {
      case 'nom':
        if (this.textMatch(target.value)) validate();
          ...
          return alert("Ton nom n'est pas valide")
        }
        break;
      case 'prenom':
        if (this.textMatch(target.value)) validate();
          ...
          this.inputState[id] = 'invalid';
          return alert("Ton prénom n'est pas valide")
        }
        break;
      case 'number':
        if (this.numberMatch(target.value)) validate();
          ...
          this.inputState[id] = 'invalid';
          return alert("Ton numéro de téléphone n'est pas valide")
        }
        break;
      case 'mail':
        if (this.mailMatch(target.value)) validate();
          ...
          this.inputState[id] = 'invalid';
          return alert("Ton mail n'est pas valide")
        }
        break;
    }

    this.inputState.day = (document.querySelector('.checked') != null) ? 'valid' : 'invalid';
    if(!this.inputState.day) return alert("N'oublie pas de cocher le.s jour.s avec lesquel.s tu seras avec nous !")
