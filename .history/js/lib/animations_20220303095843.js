...
function closeProgram(e) {
  ...
  if(e.target == closeProgramButton || e.composedPath().every(node => node != program)) {
    section1.querySelector('#invitation').classList.remove('invitation--hidden');
...
function expandIcon (selector) {
  document.querySelector('.feature--'+selector).addEventListener('mouseover', () => {
    const icon = document.querySelector('.icon--'+selector)
    icon.style.clipPath = "circle(100px at center)"
    icon.style.transition = "all .3s ease-in-out"
  })

  document.querySelector('.feature--'+selector).addEventListener('mouseout', () => {
    const icon = document.querySelector('.icon--'+selector)
    icon.style.clipPath = "circle(110px at center)"
  })
}

['bike', 'sports', 'electronics'].forEach(selector = expandIcon(selector))
